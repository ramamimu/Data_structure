...
void avl_findU(AVL *avl, int value) {
    res=avl->_root->data;
    _searchA(avl->_root, value);
}
...
int main(){
    ...
    for(int i=0; i<n; i++)
    {
        ...
        scanf("%d", &numb2);
        res=0;
        if(numb2==avlku._root->data || !avl_find(&avlku, numb2))
        {
            // cout<<res<<endl;
            printf("%d\n", res);
            continue;
